...
##
## Configure package
inside 1 root $local__container_name_lowercase "mkdir /var/log/nginx"
...
##
## Security
inside 1 root $local__container_name_lowercase "openssl req -x509 -newkey rsa:4096 -sha512 -nodes -keyout /etc/pki/nginx/private/server.key -x509 -days 36500 -out /etc/pki/nginx/server.crt -subj '/C=XX/ST=Server/L=Server/O=Server/OU=Server/CN=Server'"
inside 1 root $local__container_name_lowercase "htpasswd -cb5 /etc/nginx/.htpasswd layer_1 ${global__project_keys["[c:$local__container_name_lowercase]+[u:private]+[p:htpasswd]"]}"
